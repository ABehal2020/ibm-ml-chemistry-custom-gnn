...
class EdgeFeatures(torch.nn.Module):
    ...
    def forward(self, node_features, edge_index, edge_features):
        ...
        for i in range(edge_index.shape[1]):
            ...
            # memory leak has to do with the line below???
            edge_features[0][i] = ((original_edge_features[0][i].T + F.relu(intermediate_features_relu_input)).T).detach().clone()

            # del firstNodeIndex, secondNodeIndex, node_features_sum
            # del intermediate_node_features, intermediate_edge_feature, intermediate_features_relu_input

        del original_edge_features
...
class Features_Set2Set():
    ...
    def transform_then_concat(self, node_features, edge_features):
        ...
        del node_features_input
        del edge_features_input
